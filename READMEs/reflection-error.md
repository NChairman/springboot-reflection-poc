...
On this **error** branch, `/demo/fail` and `/demo/fail/{id}` fail with **500 Internal Server Error**. Only `/demo/success` and `/demo/success/{id}` return 200 OK.

| Endpoint | Method | Parameters | Expected outcome on this branch |
|----------|--------|------------|---------------------------------|
| `/demo/fail` | GET | `query` (required), `filter` (optional) | **500 Internal Server Error** |
| `/demo/success` | GET | `query` (required), `filter` (optional) | **200 OK** — body: `query=<value>, filter=<value>`. |
| `/demo/fail/{id}` | GET | path: `id` | **500 Internal Server Error** |
...
3.  Run the app: `./gradlew bootRun`.
    *   Test **all** endpoints above and confirm the expected outcomes: `/demo/fail` and `/demo/fail/{id}` **must** fail with **500 Internal Server Error**; `/demo/success` and `/demo/success/{id}` return 200.
